...
def unknowntags(request):
    ...
    items = (
        Unknowntag.objects.all().filter(Q(last_used__gte=cutoff)).order_by("-last_used")
    )
...
def link_tag_user(request, form, tag, user):
    try:
        ...
        newtag = Tag.objects.create(
            tag=tag.tag,
            owner=user,
            description="The card that was linked to this user on {} by {} through the self service portal".format(
                datetime.date.today(), request.user
            ),
        )
        ...
        if form.cleaned_data.get("activate_doors"):
            ...
            e, created = Entitlement.objects.get_or_create(
                active=True, permit=doors, holder=user, issuer=request.user
            )
            if created:
                e.changeReason = (
                    "Auto created during linking of unknown tag by {}".format(
                        request.user
                    )
                )
                e.save()
